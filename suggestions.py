...
def load_deck(file_name):
    ...
    deck_file_lines = file.read().split("\n")
...
def cosine_similarity(p1, p2, idf_score):
    return SparseVector.dot(p1, p2) * idf_score
...
def suggest_cards_knn(deck_list, n=10, re_suggest=False):
    deck_list = filter_deck(deck_list)
    deck_vector = compute_average_deck_vector(deck_list)

    return knn_cosine_similarity(deck_vector, n, re_suggest, deck_list)
...
def knn_cosine_similarity(deck_vector, n, re_suggest, deck_list=None):
    similarity = {}
    for card_id in allowed_cards:
        idf_score = get_idf_score(card_id)
        similarity[card_id] = cosine_similarity(deck_vector, inverted_index[card_id], idf_score)
